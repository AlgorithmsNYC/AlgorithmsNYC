...
class Graph {
  ...
  // Oriented two-dimensional array with weight : $edges[ Start point guid ][ End point guid ] = weight
  public $edges = array();
  ...
  public function random_noncrossing_edge_generation( $relationNumber = 2, $maxDist = null, $isOriented = false ) {
    $edgeNumber = 0;
    foreach( $this->distances as $guid => $distanceList ) {
      ...
      while( (list( $pointGuid, $distance ) = each( $distanceList ) ) && $edgeByVertexNumber < $relationNumber ) { 
        ...
        // Distance check
        if( !is_null( $maxDist ) && $distance > $maxDist * 1.9 ) {
        ...
        if( $lineDrawingFlag
          && !isset( $this->edges[$guid][$pointGuid] )
          && ($isOriented || !isset( $this->edges[$pointGuid][$guid] ) )
        ) {
          $this->edges[$guid][$pointGuid] = $distance;
          if( ! $isOriented )
            $this->edges[$pointGuid][$guid] = $distance;
        }
        $edgeByVertexNumber++;
        $edgeNumber++;
      }
  ...
  public function draw($sizeX = 500, $sizeY = 500, $isOriented = false) {
